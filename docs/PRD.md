...
**Rental listings preview datasets — September 8, 2026 (branch `demo/rental-listings`).** First use of the tool for data that is *not yet on DataCommon*: MAPC's rental listings database summaries for 2026 Q2, at municipal and census-tract levels. This introduced a general **preview-dataset mechanism**: catalog entries flagged `local: true` load from static JSON in `public/data/local/` instead of the API, are merged into the picker alongside the DataCommon catalog, show a "Preview — not yet published to DataCommon" badge, and hide the DataCommon deep link and CSV export (which have nothing to point at). Sibling auto-swap works across bins for preview pairs. `scripts/build-local-datasets.mjs` cleans and pivots the long-format listings CSVs wide (one row per unit; bedroom × metric columns), maps muni names to `muni_id`, merges muni-straddling tract summaries (counts summed, means count-weighted, median from the larger sample), and drops malformed or non-MAPC rows. Result: 99 MAPC munis, 659 tracts (2010 vintage). Kept on a demo branch rather than `main` until MAPC decides whether the listings data is publishable; the mechanism itself is reusable for any pre-publication dataset.
